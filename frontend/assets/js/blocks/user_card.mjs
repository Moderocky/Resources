...
export const card = (user) => dom.create(`
        <div class="w-full md:w-3/12 p-2 opacity-0 transition-opacity">
            <div aria-label="{full_name}" 
                ...
                class="select-none cursor-pointer p-1 h-full rounded-xl group sm:flex space-x-6 bg-slate-900 dark:bg-white dark:bg-opacity-10 bg-opacity-10 shadow-xl hover:bg-opacity-25 text-ellipsis overflow-hidden">
                ...
                <div class="sm:w-8/12 pl-0 p-5 h-full">
                    <div class="space-y-2 flex flex-col h-full justify-between">
                        <div class="space-y-4">
                            ...
                            ` + (user.bio != null ? `<p class="text-gray-600 dark:text-gray-400">` + user.bio + `</p>` : '') +`
                        </div>
        ...
        </div>`, {...user, name: (user.name || user.login)});
